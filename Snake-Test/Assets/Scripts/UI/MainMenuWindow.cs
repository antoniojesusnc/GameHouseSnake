...
namespace GameHouse.Snake.UI
{
    public class MainMenuWindow : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            transform.Find(MAIN_SUB).Find(HOW_TO_PLAY_BUTTON).GetComponent<Button_UI>().ClickFunc =
                () => ShowSub(Sub.HowToPlay);
            transform.Find(MAIN_SUB).Find(HOW_TO_PLAY_BUTTON).GetComponent<Button_UI>().AddButtonSounds();

            transform.Find(HOW_TO_PLAY_SUB).Find(BACK_BUTTON).GetComponent<Button_UI>().ClickFunc =
                () => ShowSub(Sub.Main);
